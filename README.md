...
of waiting on notarization. The background is generated by
`packaging/dmg-background.swift` and the window layout is recorded by
`packaging/dmg-window.applescript` — the Finder is the only thing that can write
that into an image, so the first run may ask to allow controlling Finder.
